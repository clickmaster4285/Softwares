...
export function generateStaticParams() {
  const goalParams = getAllGoalStaticParams().map((goal) => ({
    slug: goal.slug,
    service: goal.service,
    'persona-based': goal['goal-based'],
  }));
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug, service, 'persona-based': segmentSlug } = await params;
...
export default async function ServiceSegmentPage({ params }: Props) {
  const { slug, service, 'persona-based': segmentSlug } = await params;

  const goalPage = getGoalPage(segmentSlug);
  if (goalPage) {
    const resolvedService = resolveGoalServiceSlug(service);
    const baseService = getServicePage(resolvedService);

    if (!baseService) notFound();
    if (!goalPageMatchesService(goalPage, service)) notFound();
    ...
    const sections = goalPage.sections || [];
    const faqs = goalPage.faqs || [];
    const canonicalPath = `/${slug}/${service}/${segmentSlug}`;
    const url = `${siteConfig.url}${canonicalPath}`;
    const techStack = getServiceTechnologies(resolvedService);
    const displayServiceName = baseService.serviceName;
    const servicesCards = goalPage.servicesCards ?? baseService.servicesCards;
    const differentiators = goalPage.differentiators ?? baseService.differentiators;
    const processPhases = goalPage.processPhases ?? baseService.processPhases;
    const industryUseCases = goalPage.industryUseCases ?? baseService.industryUseCases;
    const pricingTiers = goalPage.pricingTiers ?? baseService.pricingTiers;
    ...
    const heroPage = {
      ...
      lead: goalPage.lead,
      highlights: goalPage.highlights,
      marketStats: goalPage.marketStats,
      currentPageLabel: goalPage.goalLabel,
      parentService: {
        label: baseService.serviceName,
        href: `/${slug}/${service}`,
      },
      boldTerms: [displayServiceName, goalPage.goalLabel],
    };
    ...
    if (goalPage.relatedGoals.length > 0) {
    ...
    const serviceJsonLd = serviceSchema(goalPage.serviceName, goalPage.metaDescription, url);
    const faqJsonLd = faqSchema(faqs.length > 0 ? faqs : defaultFaqs);

    return (
      <>
        <Script
          id={`service-schema-${goalPage.slug}`}
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(serviceJsonLd) }}
        />
        <Script
          id={`faq-schema-${goalPage.slug}`}
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(faqJsonLd) }}
        />
        <Script
          id={`breadcrumb-${goalPage.slug}`}
          type="application/ld+json"
          dangerouslySetInnerHTML={{
            __html: JSON.stringify(
              breadcrumbSchema([
                ...
                { name: goalPage.goalLabel, url: canonicalPath },
              ]),
            ),
          }}
        />
          ...
          <div className="mx-auto max-w-8xl px-16 md:px-8 lg:px-16">
            <div className="relative lg:grid lg:grid-cols-[1fr_260px] lg:gap-16">
              <main className="py-12 lg:py-10">
                ...
                {differentiators && (
                  <WhyChooseUs slug={goalPage.slug} differentiators={differentiators} />
                )}
                ...
                {goalPage.relatedGoals.length > 0 && (
                  <section id="related-goals" className="scroll-mt-24 pt-8">
                    ...
                    <div className="mt-8 grid gap-3 sm:grid-cols-2">
                      {goalPage.relatedGoals.map((goal) => (
                        <Link
                          key={goal.label}
                          href={goal.href}
