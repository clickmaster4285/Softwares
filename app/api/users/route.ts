...
export async function GET() {
  await dbConnect();
  try {
    // ✅ Await the async function to get actual token string
    const token = await getTokenFromCookies();
    if (!token) return NextResponse.json({ message: 'Unauthorized' }, { status: 401 });

    // ✅ Now token is a string, safe to pass to verifyToken
    const decoded: { id: string; email: string } = await verifyToken(token);
