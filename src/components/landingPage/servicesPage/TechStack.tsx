...
function TechCard({ item }: { item: TechItem }) {
  return (
    <div className="flex flex-col items-center justify-center gap-2 flex-shrink-0 w-28 group cursor-default">
      ...
      <span className="text-xs text-slate-500 group-hover:text-slate-800 transition-colors duration-300 text-center leading-tight">
        {item.name}
      </span>
    </div>
  );
      ...
      <section id="tech-stack" className="scroll-mt-24">
        ...
        <div className="space-y-10">
          {categoryItems.map(({ category, items }, rowIndex) => {
            ...
            return (
              <div key={category}>
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-5 pl-1">
                  {category}
                </p>
                ...
                <div className="relative overflow-hidden">
                  ...
                  <div className={`flex gap-8 w-max ${goLeft ? "marquee-left" : "marquee-right"}`}>
