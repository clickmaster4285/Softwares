...
interface SimpleMistakeCardItem {
  title: string;
  desc: string;
}
...
const getIconForTitle = (title: string): LucideIcon => {
  const iconMap: Record<string, LucideIcon> = {
    'Agile Development': AlertTriangle ,
    'Dedicated Teams': Users,
    'Transparent Communication': Zap,
    'Timezone Flexibility': Clock,
    'Scalable Architecture': AlertTriangle ,
    'Ongoing Support': Clock,
  };
  return iconMap[title] || AlertTriangle ;
};

export function MistakeCard({ countryName, items, subtitle = "Competitive differentiation that sets us apart" }: MistakeCardProps) {
            ...
            items.map((item, index) => {
              ...
              return (
                <motion.div
                  ...
                  whileHover={{
                    scale: 1.05,
                    boxShadow: '0 25px 35px -15px rgba(239,68,68,0.3)',
                    ...
                    >
                      <div className="relative inline-block">
                        ...
                        <div className="relative w-12 h-12 flex items-center justify-center">
                          <Icon className="w-6 h-6 text-red-600 group-hover:text-red-600 transition-colors duration-300" strokeWidth={1.5} />
                        </div>
                      </div>
                    </motion.div>
